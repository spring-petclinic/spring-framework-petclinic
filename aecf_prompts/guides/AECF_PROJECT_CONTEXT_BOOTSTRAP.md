...
En `aecf_prompts` conviene usarlo como bootstrap de una sola vez y, a partir de ese resultado, consolidar o refrescar `.aecf/documentation/AECF_PROJECT_CONTEXT.md` para consumo manual por los prompts.
...
Para cada ejecucion real de un `TOPIC`, el bundle prompt-only tambien debe crear `<DOCS_ROOT>/<user_id>/<TOPIC>/AECF_RUN_CONTEXT.json` con el `output_language` congelado para todas las fases del flujo. `DOCS_ROOT` usa `artifacts_path` de `.aecf/user_settings.json` si existe; si no, `AECF_PROMPTS_DOCUMENTATION_PATH`; si no, cae por defecto en `<workspace>/.aecf/documentation` y debe crearse si todavia no existe.
...
- `.aecf/runtime/context/AECF_PROJECT_CONTEXT_HUMAN.yaml`
- `.aecf/runtime/context/AECF_PROJECT_CONTEXT_RESOLVED.json`
- `.aecf/documentation/<user_id>/AECF_TOPICS_INVENTORY.json`
- `.aecf/documentation/<user_id>/AECF_TOPICS_INVENTORY.md`
- `.aecf/documentation/<user_id>/AECF_CHANGELOG.md`

Si durante una ejecucion prompt-only aparece otro `AECF_PROJECT_CONTEXT` fuera de `.aecf/documentation/`, o si `AECF_CHANGELOG` / `AECF_TOPICS_INVENTORY` aparecen fuera de `.aecf/documentation/<user_id>/`, la salida no sigue el contrato del bundle.

### Contexto tecnico dinamico

`aecf_codebase_intelligence` o `aecf_set_stack` generan en `documentation/context/`:
...
1. `documentation/context/*` debe reutilizarse como capa estructurada global del repo;
...
`documentation/context/*` = inteligencia global reutilizable.
...
El mayor ahorro aparece en los flujos multifase que reutilizan `.aecf/documentation/AECF_PROJECT_CONTEXT.md` en varias sesiones.
...
1. Mantener `.aecf/documentation/AECF_PROJECT_CONTEXT.md` como fuente humana legible.
2. Mantener `AECF_CHANGELOG.md` y `AECF_TOPICS_INVENTORY.{json,md}` en `.aecf/documentation/<user_id>/`.

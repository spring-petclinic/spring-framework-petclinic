...
| 🚀 **Bootstrap y Setup** | `aecf_project_context_generator` · (hidden) · `aecf_new_project` · `aecf_codebase_intelligence` · `aecf_set_stack` · `aecf_surface_discovery` |
...
| **Executive Summary** | `aecf_executive_summary` | Generates consolidated executive summary of all documents of a TOPIC | When an overall executive summary is required from `<DOCS_ROOT>/<user_id>/{{TOPIC}}/`; Mandatory explicit TOPIC |
...
```
aecf_project_context_generator → aecf_maturity_assessment
```

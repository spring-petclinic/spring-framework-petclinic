...
4. Preferred MCP-first mappings are: `@aecf list commands` -> `aecf_list_commands`; `@aecf show commands` -> `aecf_show_commands`; `@aecf list skills` -> `aecf_list_skills`; `@aecf list prompts` -> `aecf_list_prompts`; `@aecf list topics` -> `aecf_list_topics`; `@aecf show prompt=<name>` -> `aecf_show_prompt`; `@aecf show guide=<name>` -> `aecf_show_guide`; `@aecf context` or `@aecf context examine` -> `aecf_context_examine`; `@aecf find skills prompt=...` -> `aecf_find_skills`; `@aecf status [topic=...]` -> `aecf_status`; `@aecf memory list` -> `aecf_memory_list`; `@aecf memory add ...` -> `aecf_memory_add`; `@aecf memory search=...` -> `aecf_memory_search`; `@aecf memory update id=... text=...` -> `aecf_memory_update`; `@aecf memory delete id=...` -> `aecf_memory_delete`.
...
8. Before executing a skill, load `<DOCS_ROOT>/AECF_PROJECT_CONTEXT.md` if it exists. Default prompt-only location: `.aecf/documentation/AECF_PROJECT_CONTEXT.md`.
9. When `documentation/context/` already contains structured intelligence artifacts, treat them as the reusable repository layer for downstream prompt-only skills. Prefer deriving a compact execution-scoped context block from `STACK_JSON.json`, `AECF_CONTEXT_KEYS.json`, `AECF_SYMBOL_INDEX.json`, `AECF_ARCHITECTURE_GRAPH.json`, `AECF_ENTRY_POINTS.json`, `AECF_MODULE_MAP.json`, `AECF_CODE_HOTSPOTS.json`, and `AECF_DYNAMIC_PROJECT_CONTEXT.md` instead of re-scanning the whole repository.
10. Do not paste the full `documentation/context/` JSON set into every phase prompt by default. Filter by `TOPIC`, active `surface`, skill intent, and phase scope unless a skill contract explicitly requires the raw artifacts.
11. For `DISCOVERY_FIRST` prompt-only skills, build a topic-scoped `WORKING_CONTEXT` from the current `TOPIC`, `AECF_RUN_CONTEXT.json`, the relevant `documentation/context/*` artifacts, and any bounded fresh evidence. `WORKING_CONTEXT` is execution-scoped evidence, not the global repository cache.
